...
class Config:
    ...
    web_path = "/a"  # GitHub Webhook的地址，填写`/a`，则内网地址为`http://127.0.0.1:<NB端口>/a`，`NB端口`在`.env.dev`中的`PORT`

    bot = ["3336149819"]  # Bot的QQ号，此处请只填写一个，用`str`存在`list`中。

    platform = True  # 平台标识，`True`为Linux，`False`为`Windows`

    owner = ["122392432"]  # Bot主人，可以有多个，用`str`存在`list`中
    ...
    cqhttp = "https://go-cqhttp.org"  # CQHTTP地址，参考`https://go-cqhttp.org`。
    ...
    jx3api_wslink = "https://vip.jx3api.com"  # JX3API WebSocket地址

    jx3api_wstoken = "https://vip.jx3api.com"  # JX3API WebSocket Token

    jx3api_globaltoken = "https://vip.jx3api.com"  # JX3API API Token
    ...
    repo_name = "codethink-cn/Inkar-Suki"  # 该`Inkar-Suki`的副本的来源，若从主仓库克隆，则填写`codethink-cn/Inkar-Suki`，若为fork之后克隆的仓库，则填写`<你的GitHub用户名>/Inkar-Suki`
